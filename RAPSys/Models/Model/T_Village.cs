//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace RAPSys.Models.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class T_Village
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public T_Village()
        {
            this.T_LandVillage = new HashSet<T_LandVillage>();
            this.T_Property = new HashSet<T_Property>();
        }
    
        public int VillageId { get; set; }
        public string VillageName { get; set; }
        public string OldName { get; set; }
        public int VillageStatus { get; set; }
        public int RegionId { get; set; }
        public Nullable<int> VillageCategory { get; set; }
        public string Administration { get; set; }
        public string Sponsorship { get; set; }
        public string VillageChief { get; set; }
        public string LandChief { get; set; }
        public string GroupmentChief { get; set; }
        public Nullable<System.DateTime> GPSDate { get; set; }
        public Nullable<int> VillagePoint { get; set; }
        public Nullable<bool> RiverWaterUse { get; set; }
        public string Commentaires { get; set; }
        public System.DateTime Created { get; set; }
        public string CreatedBy { get; set; }
        public System.DateTime Updated { get; set; }
        public string UpdatedBy { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<T_LandVillage> T_LandVillage { get; set; }
        public virtual T_List T_List { get; set; }
        public virtual T_List T_List1 { get; set; }
        public virtual T_Point T_Point { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<T_Property> T_Property { get; set; }
        public virtual T_Region T_Region { get; set; }
    }
}
